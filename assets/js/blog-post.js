...
const applyCategoryClass = (element, categoryKey) => {
  if (!element || !categoryKey) return;
  element.classList.forEach((cls) => {
    if (cls.startsWith('category-')) {
      element.classList.remove(cls);
    }
  });
  element.classList.add(`category-${categoryKey}`);
};
...
const renderPost = () => {
  ...
  if (categoryEl) {
    ...
    applyCategoryClass(categoryEl, post.categoryKey);
  }

  const dateEl = document.getElementById('post-date');
  if (dateEl) {
    dateEl.innerHTML = `<i class="far fa-calendar-alt"></i> ${formatDate(post.date)}`;
  }
  ...
  if (readTimeEl) {
    readTimeEl.innerHTML = `<i class="far fa-clock"></i> <span>${post.readTime}</span>`;
  }
  ...
  if (shareTwitter) shareTwitter.href = `https://twitter.com/intent/tweet?text=${shareText}&url=${currentUrl}`;
